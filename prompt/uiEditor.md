...
- Sprite 与 Label 同属 `componentType: 1`，互斥；Opacity 为 `2`；SimpleList 为 `3`；Button 为 `4`；LangSprite 为 `5`；LangLabel 为 `6`；ImgToFile 为 `7`；**TemplateComponent 为 `1000`，仅允许挂在 Root**，不导出为 Prefab 组件。
...
- **添加组件**（Inspector 与 PSD 括号标注共用 `mountComponentOnNode`）：
  ...
  - `ImgToFileComponent`：**仅当本节点已有 `SpriteComponent`** 才可添加（Inspector 不列出；PSD 括号标注忽略并告警）。**不**自动补 Sprite。去掉 Sprite 时一并去掉 ImgToFile。
...
- 【撤销】/【重做】：顶栏按钮；快捷键 `Ctrl+Z` / `Ctrl+Y`。

### 3.1.1 顶栏布局（必须）
当前**显示中**的功能按钮在窗口变窄时仍须全部可见、可点；禁止裁切、横向滚动藏按钮、或收进「更多」菜单。
- 顶栏允许换行增高；换行后**左对齐、同行垂直居中、行列间距相同**（上下对齐）。**无组间分割线**。
- 每个功能按钮为独立项（含 split 的导入/导出），换行时不把一组按钮当成不可拆的整块。
- 窄屏可减小按钮左右内边距，**文案与职责不变**。
- 右侧项目名、当前文件路径、保存状态**优先收缩省略**（`truncate`），不得把按钮挤出视口。
...
- **上：节点树**（`el-tree` + `draggable`）
  ...
  - **多选（必须）**：`show-checkbox` + `check-strictly`（勾选不级联）。单击 = 只选该项（Inspector / 画布当前节点）；**Ctrl/⌘+单击**或勾选框 = 切换加入集合。Root **不**进入多选（隐藏其勾选框）。标题显示已选数量。
  - **多选拖拽**：拖动集合中的任一节点时，把集合里互不为子孙的节点一并移到落点（`inner` 成为目标子级；`prev`/`next` 与目标同级）。子节点随已选祖先移动，不必再单独勾。禁止拖进自身或子孙；Root 不可拖。
...
- **下：项目文件树**
  ...
  - **多选（必须）**：同上勾选 / Ctrl/⌘+单击。拖到文件夹上（`inner`）或文件/文件夹旁（`prev`/`next`，进入其父目录）= **磁盘移动**（复制后删源）。目标已有同名项则失败并提示，并刷新树。不可移入自身或子孙。祖先已选则只移祖先。已打开的 UI 若被移动则更新 `currentFilePath` / 句柄；资源过滤目录若被移动则同步。右键删除对集合整批（不可撤销）。
...
- **稳定 UUID**：由种子字符串经可复现哈希生成 RFC 风格 UUID；同一种子多次导出不变。推荐算法（可原样实现）：
  ...
  - 非 `UI/` 子目录图种子：`cocos-image:{subdir}:{项目相对路径}`（如 `cocos-image:UI/zh:…`、`cocos-image:UI/img:…`；UUID version **4**，与 Creator 原生资源一致）；该图 `.meta` 以及该子目录链上的 `.meta`（`UI/img.meta`、嵌套则为 `UI/a.meta` + `UI/a/b.meta`）的 `imported` 必须为 **false**；此时 `UI.meta` / Prefab.meta 也是 `imported: false`，让资源库立刻按给定 UUID 导入。先写图片再写子目录 `.meta`。禁止 `imported: true` 却无 `library/` 缓存，否则编辑器要手动刷新才显示。
- 覆盖导出必须先删除旧包目录，避免 `UI/` 残留旧 UUID 与子目录新文件冲突。
...
2. 新建子节点、树拖拽排序、画布点选最深层、拖拽改 xy、四角改 wh、Root 不可删不可缩放。节点树 / 文件树可多选：Ctrl+点或勾选；多选拖到目标；节点多选复制/删除；文件多选拖到其它文件夹（磁盘移动）。底部图片双击放大，Esc 关闭。
