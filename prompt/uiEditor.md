...
- **图层顺序（重要）**：**禁止用 `zIndex` 控制 PSD 层级**（导入时不写入递增 zIndex；画布不按 zIndex 排序）。`ag-psd` 的 `children` 为 Photoshop 面板自上而下（`[0]`=顶层，末尾常为 BG）。导入时按绘制顺序**自下而上生成** `children`：`children[0]`=BG（最底层）… `children[n-1]`=顶层；画布严格按该数组 `addChild` 顺序绘制。图层组内同样处理；组内子节点为相对父节点坐标。